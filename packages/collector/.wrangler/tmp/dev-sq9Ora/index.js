...
// .wrangler/tmp/bundle-WdR8pe/strip-cf-connecting-ip-header.js
...
function parseUserAgent(ua) {
  ...
  if (/edg\//i.test(ua)) {
    const match2 = ua.match(/edg\/([\d.]+)/i);
    browser = `Edge ${match2 ? match2[1].split(".")[0] : ""}`;
  } else if (/chrome|crios/i.test(ua) && !/opr|opera/i.test(ua)) {
    const match2 = ua.match(/(?:chrome|crios)\/([\d.]+)/i);
    browser = `Chrome ${match2 ? match2[1].split(".")[0] : ""}`;
  } else if (/firefox|fxios/i.test(ua)) {
    const match2 = ua.match(/(?:firefox|fxios)\/([\d.]+)/i);
    browser = `Firefox ${match2 ? match2[1].split(".")[0] : ""}`;
  } else if (/safari/i.test(ua) && !/chrome/i.test(ua)) {
    const match2 = ua.match(/version\/([\d.]+)/i);
    browser = `Safari ${match2 ? match2[1].split(".")[0] : ""}`;
...
__name(insertBatchEvents, "insertBatchEvents");
async function getOverviewStats(db, appId, startTime, endTime) {
...
__name(getOverviewStats, "getOverviewStats");
async function getTrendStats(db, appId, startTime, endTime) {
  const sql = `
    SELECT 
...
__name(getTrendStats, "getTrendStats");
async function getDistributionStats(db, appId, startTime, endTime) {
  const browserSql = `
...
__name(getDistributionStats, "getDistributionStats");
async function getEventsList(db, appId, eventType, keyword, userId, startTime, endTime, page = 1, pageSize = 20) {
...
__name(getEventsList, "getEventsList");
async function getTopPagesStats(db, appId, startTime, endTime, limit = 10) {
  const sql = `
...
__name(getTopPagesStats, "getTopPagesStats");

// src/routes/track.ts
var trackRoute = new Hono2();
...
trackRoute.post("/", async (c) => {
  ...
  try {
    ...
    const count3 = await insertBatchEvents(c.env.DB, normalizedEvents);
    return c.json({
      code: 200,
      message: "\u4E0A\u62A5\u6210\u529F",
      data: { inserted: count3 }
    });
  } catch (err) {
    const error3 = err;
    console.error("\u57CB\u70B9\u63A5\u6536\u9519\u8BEF:", error3);
    return c.json({ code: 500, message: "\u670D\u52A1\u5668\u5199\u5165\u5F02\u5E38", error: error3.message }, 500);
  }
});
var track_default = trackRoute;

// src/routes/stats.ts
var statsRoute = new Hono2();
function parseTimeParams(c) {
...
__name(parseTimeParams, "parseTimeParams");
...
statsRoute.get("/events", async (c) => {
  ...
  const startTime = c.req.query("startTime") ? parseInt(c.req.query("startTime"), 10) : void 0;
  const endTime = c.req.query("endTime") ? parseInt(c.req.query("endTime"), 10) : void 0;
...
var stats_default = statsRoute;

// src/routes/apps.ts
var appsRoute = new Hono2();
...
app.route("/api/v1/track", track_default);
app.route("/api/v1/stats", stats_default);
app.route("/api/v1/apps", apps_default);
...
// .wrangler/tmp/bundle-WdR8pe/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-WdR8pe/middleware-loader.entry.ts
